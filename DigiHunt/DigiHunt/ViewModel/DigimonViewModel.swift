...
class DigimonViewModel: ObservableObject {
    ...
    private var coreDataManager: CoreDataManagerAction

    init(apiManager: APIService, coreDataManager: CoreDataManagerAction) {
...
//            print(list)
            ...
            Task {
                do {
                    try await self?.coreDataManager.saveDataIntoDatabase(
                        digimonList: list)
                    self?.getSQLitePath() 
                } catch {
    ...
    func favoriteDigimon(digimon: Digimon) {
        if let index = favoriteDigimons.firstIndex(where: {
            $0.id == digimon.id
        }) {
    ...
    private func getSQLitePath(){
        ...
        let path = url.appendingPathComponent("LocalStorage")
        print(path)
    }
}
